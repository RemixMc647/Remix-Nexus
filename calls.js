...
(function () {
  ...
  function makeGroupPeer(peerUserId) {
    const pc = makePeerConnection(
      (candidate) => socket.emit('groupcall:signal', { callId: groupCall.callId, toUserId: peerUserId, data: { kind: 'candidate', candidate } }),
      (stream) => { renderGroupRemote(); }
    );
    groupCall.localStream.getTracks().forEach(track => pc.addTrack(track, groupCall.localStream));
    return pc;
  }

  // Same "show whoever most recently sent media" simplification as
  // renderRoomRemote — reasonable for a handful of people in a call.
  function renderGroupRemote() {
    if (!groupCall) return;
    const streams = Array.from(groupCall.peers.values()).map(p => p.stream).filter(Boolean);
    remoteVideoEl.srcObject = streams.length ? streams[streams.length - 1] : null;
    callSubtitleEl.textContent = `${groupCall.peers.size + 1} in call`;
  }
  ...
  async function handleGroupCallParticipants({ callId, participants } = {}) {
    ...
    for (const p of participants) {
      ...
      const pc = makeGroupPeer(p.userId);
      groupCall.peers.set(p.userId, { pc, username: p.username, avatar: p.avatar, stream: null });
      const offer = await pc.createOffer();
      await pc.setLocalDescription(offer);
  ...
  async function handleGroupCallSignal({ callId, data, fromUserId } = {}) {
    ...
    if (!peer) {
      const pc = makeGroupPeer(fromUserId);
      peer = { pc, username: '', avatar: '', stream: null };
      groupCall.peers.set(fromUserId, peer);
      pc.ontrack = (e) => { peer.stream = e.streams[0]; renderGroupRemote(); };
    }
